package com.lokesh.01_Maven_App.web;
...
class SampleControllerTest {
	...
	void whenRetrieveAllWithoutAuthThenUnauthenticated() throws Exception {
		mockMvc.get("/sample").andExpect(status().isUnauthorized());
	}

	@Test
	@OpenId()
	void whenRetrieveAllWithAuthThenOk() throws Exception {
		mockMvc.get("/sample").andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)));
	}
